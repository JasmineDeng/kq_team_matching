...
def test_convert_alias_to_name() -> None:
    ...
    player_pool = PlayerNamePool(all_players, aliases)
...
def test_player_pool_name_get_contains() -> None:
    all_names = ["A", "B", "C", "D", "E"]
    all_players = [fake_player(name) for name in all_names]
    player_pool = PlayerNamePool(all_players, [{"B", "BEE"}, {"C", "SEE", "SEA"}])
    ...
    player_pool_disjoint = PlayerNamePool(all_players[:3] + [fake_player("F")])
    ...
    with pytest.raises(ValueError):
        PlayerNamePool(all_players + [fake_player("A")])
